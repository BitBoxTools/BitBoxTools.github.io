...
`),C=5;let g=0,h=[];const v=()=>{for(;h.length>0&&g<C;){const l=h.shift();g+=1,l().finally(()=>{g-=1,v()})}};for(let l of n){if(l=l.trim(),l.length!==66&&l.length!==64){F.error({message:"错误",description:"请输入正确的stark地址(64位)",duration:1});continue}l.startsWith("0x")||(l="0x"+l);const w=()=>new Promise(async(E,O)=>{try{a(S=>{const b=[...S];if(b.findIndex(x=>x.address===l)===-1){const x={key:j.toString(),address:l,result:"pending"};j++,b.push(x)}return b});const A=await J(l);a(S=>{const b=[...S],p=b.findIndex(x=>x.address===l);return p!==-1&&(b[p]={...b[p],...A}),b}),E()}catch(A){O(A)}});h.push(w)}for(v();g>0||h.length>0;)await new Promise(l=>setTimeout(l,100));F.success({message:"成功",description:"批量添加完成",duration:1})}catch(e){F.error({message:"错误",description:e.message,duration:1})}finally{d.resetFields(),m([]),o(!1)}},R=async e=>{const n=e?[e]:_;if(!n.length){F.error({message:"错误",description:"请先选择要刷新的地址",duration:1});return}u(!0);try{let g=0,h=[];const v=()=>{for(;h.length>0&&g<5;){const l=h.shift();g+=1,l().finally(()=>{g-=1,v()})}};for(let l of n){const w=t.findIndex(E=>E.key===l);if(w!==-1){const E=()=>new Promise(async(O,A)=>{try{a(b=>{const p=[...b];for(let x in p[w])x!=="address"&&x!=="name"&&x!=="key"&&(x==="result"?p[w][x]="pending":p[w][x]=null);return p});const S=await J(t[w].address);a(b=>{const p=[...b];return p[w]={...p[w],...S},localStorage.setItem("stark_addresses",JSON.stringify(p)),p}),O()}catch(S){A(S)}});h.push(E)}}for(v();g>0||h.length>0;)await new Promise(l=>setTimeout(l,100));F.success({message:"完成",description:"刷新地址数据完成",duration:1})}catch(C){F.error({message:"错误",description:C.message,duration:1})}finally{u(!1),e||m([])}},ee=async()=>{if(!_.length){F.error({message:"错误",description:"请先选择要删除的地址",duration:1});return}const e=t.filter(n=>_.includes(n.key)).map(n=>n.address);await G("starkTransactions",e),a(t.filter(n=>!_.includes(n.key))),localStorage.setItem("stark_addresses",JSON.stringify(t.filter(n=>!_.includes(n.key)))),m([])},te=()=>{pe(t,"starkInfo")};k.useState(null);const ne={selectedRowKeys:_,onChange:e=>{m(e)}};return c.jsx("div",{children:c.jsxs(De,{children:[c.jsx(ie,{title:"批量添加地址",open:i,onOk:Z,onCancel:()=>{r(!1),d.resetFields()},okText:"添加地址",cancelText:"取消",width:800,children:c.jsx(q,{form:d,layout:"vertical",children:c.jsx(q.Item,{label:"地址",name:"addresses",rules:[{required:!0,validator:(e,n)=>{const C=n.split(`
